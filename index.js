...
if (process.argv[2] == "--create") {
  ...
  rl.question("Please paste your offer:\n", function (offer) {
    var json = Buffer.from(offer, 'base64').toString('ascii');
    getOffer(json);
  });
}
...
function doShowAnswer() {
  ...
  console.log("\n\nHere is your answer:");
  console.log(Buffer.from(JSON.stringify(answer)).toString('base64') + "\n\n");
}
  ...
  pc.onicecandidate = function (candidate) {
    ...
    if (candidate.candidate == null) {
      ...
      rl.question("Please paste your answer:\n", function (answer) {
        var json = Buffer.from(answer, 'base64').toString('ascii');
        getAnswer(json);
      });
    }
  }
